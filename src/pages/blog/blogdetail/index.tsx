...
const BlogDetail: React.FC = () => {
  ...
  const author = useSelector((state: RootState) => (post ? state.posts.authors[post.authorId._id] : null));
  ...
  useEffect(() => {
    ...
    if (!token) {
      toast.error('Bạn cần đăng nhập để xem bài viết này.');
      navigate('/login');
      return;
    }
    ...
    if (id) {
      dispatch(fetchPostById(id)).then((result) => {
        ...
        if (postData && typeof postData.authorId === 'object') {
          ...
          dispatch(fetchRelatedPosts({ tags: postData.tags, excludeId: postData._id })).then((relatedResult) => {
  ...
  return (
    <div className=" min-h-screen text-white">
      <div className="container mx-auto px-[10%] ">
        {post && (
          <article className="bg-white rounded-b-3xl shadow-lg overflow-hidden text-gray-800">
            ...
            <div className="p-8">
              ...
              <div className="flex items-center mb-6">
                {author && (
                  <div className="flex items-center">
                    <img src={author.avatar || 'https://via.placeholder.com/50'} alt={author.name} className="w-12 h-12 rounded-full border-2 border-gray-300 mr-4" />
                    <div>
              ...
              <p className="text-lg leading-relaxed text-gray-700 mb-6">{post.content}</p>
              <div className="flex flex-wrap gap-3">
                {post.tags.map((tag, index) => (
                  <span key={index} className="bg-blue-200 text-blue-800 text-sm font-medium px-3 py-1 rounded-full border border-blue-300 hover:bg-blue-300 transition">
                    {tag}
                  </span>
                ))}
              </div>
        ...
        <div className="mt-12">{post && <CommentBlog postId={post._id} />}</div>
        <div className="pt10 pb-20">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {relatedPosts.slice(0, 2).map((relatedPost) => (
                  ...
                  >
                    ...
                    />
                    <div className="p-4">
                      <h3 className="font-bold text-lg mb-2 text-gray-800">{relatedPost.title}</h3>
                      <p className="text-sm text-gray-500 overflow-hidden text-ellipsis whitespace-nowrap">{relatedPost.content}</p>
                      <p className="text-sm text-gray-500">CreateAt: {new Date(relatedPost.createdAt).toLocaleDateString()}</p>
                    </div>
                  </div>
                ))}
              </div>
