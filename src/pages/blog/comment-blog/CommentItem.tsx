...
import { deleteComment, updateComment, replyToComment, fetchCommentsByPostId } from '@/lib/api/redux/commentSlice';
...
const CommentItem: React.FC<CommentItemProps> = ({ comment, comments, currentUser, postId, level = 0 }) => {
      ...
      {childComments.length > 0 && (
        <div className={`mt-4  rounded-md p-4`} style={{ marginLeft: level * 20 }}>
          {childComments.map((child) => (
            <CommentItem key={child._id} comment={child} comments={comments} currentUser={currentUser} postId={postId} level={level + 1} />
          ))}
        </div>
      )}
    </div>
