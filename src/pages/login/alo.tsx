...
import { login } from '@/lib/api/redux/authSlice';  // Import action login từ authSlice
import { RootState, AppDispatch } from '@/lib/api/store';  // Import store types

const LoginPage = () => {
  const dispatch: AppDispatch = useDispatch();  // Sử dụng dispatch từ Redux
  const { loading, error } = useSelector((state: RootState) => state.auth);  // Lấy trạng thái loading và error từ Redux
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    dispatch(login({ email, password }));  // Gửi action login
  };
